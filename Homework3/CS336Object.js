...
{
  var move = this.getMatrix();
  move.multiply(new THREE.Matrix4().makeTranslation(0,0,-distance));
  this.position.setFromMatrixPosition(move);
  this.matrixNeedsUpdate = true;
};
...
{
  var move = this.getMatrix();
  move.multiply(new THREE.Matrix4().makeTranslation(distance,0,0));
  this.position.setFromMatrixPosition(move);
  this.matrixNeedsUpdate = true;
};
...
{
  var move = this.getMatrix();
  move.multiply(new THREE.Matrix4().makeTranslation(0,distance,0));
  this.position.setFromMatrixPosition(move);
  this.matrixNeedsUpdate = true;
};
...
{
  this.rotation.multiply(new THREE.Matrix4().makeRotationX(toRadians(degrees)));
  this.matrixNeedsUpdate = true;
};
...
{
  this.rotation.multiply(new THREE.Matrix4().makeRotationY(toRadians(degrees)));
  this.matrixNeedsUpdate = true;
};
...
{
  this.rotation.multiply(new THREE.Matrix4().makeRotationZ(toRadians(degrees)));
  this.matrixNeedsUpdate = true;
};
...
{
  this.setScale(1,1,1);
  var move = new THREE.Matrix4().makeRotationAxis(new THREE.Vector3(x,y,z).normalize(),toRadians(degrees));
  this.rotation.multiply(move);
  this.matrixNeedsUpdate = true;
};
...
{
  var rPrime = 1*Math.sin(pitch);
  var move = new THREE.Matrix4().makeRotationAxis(new THREE.Vector3(rPrime*Math.sin(head),1*Math.cos(pitch),rPrime*Math.cos(head)).normalize(),toRadians(degrees));
  this.rotation.multiply(move);
  this.matrixNeedsUpdate = true;
};
...
{
  this.rotation.premultiply(new THREE.Matrix4().makeRotationY(toRadians(degrees)));
  this.matrixNeedsUpdate = true;
};
...
CS336Object.prototype.lookAt = function(x, y, z)
{
  ...
  var eye = new THREE.Vector3().setFromMatrixPosition(move);
  var target = new THREE.Vector3(x,y,z).normalize();
  var up = new THREE.Vector3(0,1,0);
  
  this.rotation.premultiply(new THREE.Matrix4().lookAt(eye,target,up));
  this.matrixNeedsUpdate = true;
};

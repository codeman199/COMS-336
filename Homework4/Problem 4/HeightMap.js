...
HeightMap.prototype.generateNormals = function()
{
  ...
  // The normal vector for vertex with index a will
  ...
  // Use a cross product to find the face normal, and normalize it.
  ...
  let indices = new Float32Array(this.vertices).fill(0);
  
  for(let i = 0; i < this.meshIndices.length - 1; i+=3){
	
	let idx1 = this.meshIndices[i];
	let idx2 = this.meshIndices[i+1];
	let idx3 = this.meshIndices[i+2];
	
	let a1 = this.vertices[(3*idx1)];
	let a2 = this.vertices[(3*idx1)+1];
	let a3 = this.vertices[(3*idx1)+2];
	
	let b1 = this.vertices[(3*idx2)];
    let b2 = this.vertices[(3*idx2)+1];
	let b3 = this.vertices[(3*idx2)+2];
	  
    let c1 = this.vertices[(3*idx3)];
    let c2 = this.vertices[(3*idx3)+1];
    let c3 = this.vertices[(3*idx3)+2];
	
	let cross = []
	cross = crossProduct(a1, a2, a3, b1, b2, b3, c1, c2, c3);
	
	indices[3*idx1] += cross[0];
	indices[(3*idx1)+1] += cross[1];
	indices[(3*idx1)+2] += cross[2];
	
	indices[3*idx2] += cross[0];
	indices[(3*idx2)+1] += cross[1];
	indices[(3*idx2)+2] += cross[2];
	
	indices[3*idx3] += cross[0];
	indices[(3*idx3)+1] += cross[1];
	indices[(3*idx3)+2] += cross[2];
  } 
  
  for(let i = 0; i < this.meshIndices.length - 1; i++){  
	let idx1 = this.meshIndices[i];
	
	let normals = new THREE.Vector3(indices[3*idx1],indices[3*idx1+1],indices[3*idx1+2]);
	normals.normalize();
	
	indices[3*idx1] = normals.x;
	indices[3*idx1+1] = normals.y;
	indices[3*idx1+2] = normals.z;
  }
